...
router.post('/chat', async (req, res) => {
    ...
    try {
        ...
        if (error.message.includes('401') || error.message.includes('invalid_api_key'))
            statusMsg = "Identity Verification Failed: The Llama API Key provided appears to be invalid or missing.";
        if (error.message.includes('429'))
            statusMsg = "High Traffic Alert: Llama 3 is processing too many requests. Please wait a moment while I clear the queue.";
