...
router.put('/profile', verifyToken, async (req, res) => {
    try {
        if (req.user.id === 1)
            return res.json({ message: 'Admin profile cannot be modified', user: { name: 'Identity Admin', role: 'admin' } });
        const { name, bio, location, picture } = req.body;
        const user = await User.findByIdAndUpdate(req.user.id, { name, bio, location, picture }, { new: true });
...
router.post('/reading-progress', verifyToken, async (req, res) => {
    try {
        if (req.user.id === 1)
            return res.json({ message: 'Progress saved (Simulation)' });
        const { bookId, paperId, progress, status } = req.body;
        const user = await User.findById(req.user.id);
        const existing = user.readingProgress.find(p => (bookId && p.bookId?.toString() === bookId) ||
